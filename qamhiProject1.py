class Random:
    ...
    def next(self):
        ...
        return nxt
    def choose(self, max):
        z = self.next(self.rand)%max
class Mumble:
    seed = 0
    min = 0
    def __init__(self, sed, choosers):
        a = 7**5
        b = 2**31
        self.seed = sed
        self.rand = (a*seed)%(b-1)
    def make(self, max):
        ...
        word = bank(i-1)
        j = 1
        while j <= min + Random.choose(c, max - min):
            y = choosers[bank(i-1)]
            x = Random.choose(c, y(0))
            while x > 0:
                x -= bank(i)
                i += 2
            word += bank(i-1)
